  ...
  const fetchMovies = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.themoviedb.org/3/search/movie?query=${query}&api_key=${process.env.EXPO_PUBLIC_KEY}`
      );
    ...
    <View style={styles.container}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          columnWrapperStyle={styles.row} // Space items horizontally in each row
        />
      )}
    </View>
  );
